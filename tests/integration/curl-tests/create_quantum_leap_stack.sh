...
#
# First create the network that is required for this stack
#
json_data=$(jq -n \
		--arg ca "$cacert" \
		--arg cert "$cert" \
...
#
# Then create the second network that is required for this stack
#
json_data=$(jq -n \
		--arg ca "$cacert" \
		--arg cert "$cert" \
...
echo "Creating network $NETWORKNAME2"
networks_response=$(curl -X POST \
  http://$MASTERMIND_SERVER:$MASTERMIND_PORT/v1/create_network \
   -H "Content-Type: Application/json" \
   --data "$json_data" )
...
#
# Finally, create the stack itself...
#
json_data=$(jq -n \
		--arg ca "$cacert" \
		--arg cert "$cert" \
